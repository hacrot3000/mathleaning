<!-- Exercise content: Luyện Tập Trị Tuyệt Đối -->
<h1><?php echo $lang['practice_absolute_value']; ?></h1>
...
<script type="text/javascript">
    ...
    var storage = createLocalStorageManager('trituyetdoi');
    var problemCountManager = createProblemCountManager('trituyetdoi');
    ...
    function generateMixedNumberForAbsolute(wholeMin, wholeMax, numMax, denMin, denMax) {
    ...
    function generateOperand(config) {
        ...
        if (type === 'integer') {
            ...
            return generateMixedNumberForAbsolute(
    ...
    function generateNewProblem() {
        ...
        var operands = [];
        var hasAbsolute = []; // Boolean array: true if operand has absolute value
        var operators = [];
        ...
        for (var i = 0; i < numOperands; i++) {
            ...
            // Generate absolute value flag
            if (Math.random() < config.absolute_probability) {
                hasAbsolute.push(true);
            } else {
                hasAbsolute.push(false);
            }
        ...
        // Ensure at least one operand has absolute value (this is an absolute value exercise!)
        var hasAnyAbsolute = hasAbsolute.some(function(a) { return a === true; });
        if (!hasAnyAbsolute) {
            // Randomly select one operand to add absolute value
            var randomIndex = getRndInteger(0, numOperands - 1);
            hasAbsolute[randomIndex] = true;
        }
        ...
        for (var i = 0; i < operands.length; i++) {
            ...
            var value = hasAbsolute[i] ? Math.abs(baseValue) : baseValue;
            values.push(value);
        }
    ...
    function displayProblem() {
        ...
        for (var i = 0; i < currentProblem.operands.length; i++) {
            var operandLatex = formatOperandLatex(currentProblem.operands[i], false);
            
            // Add absolute value if exists
            if (currentProblem.hasAbsolute[i]) {
                // Wrap operand in absolute value bars
                operandLatex = '\\left|' + operandLatex + '\\right|';
            } else if (i > 0) {
                // Add parentheses for negative numbers when NOT using absolute value
    ...
    function saveProblemToHistoryLocal(skipped) {
        ...
        for (var i = 0; i < currentProblem.operands.length; i++) {
            ...
            if (currentProblem.hasAbsolute[i]) {
                ...
                // Add parentheses for negative fractions/mixed numbers after operators
                var operandValue = calculateOperand(currentProblem.operands[i]);
                if (operandValue < 0 && (currentProblem.operands[i].type === 'fraction' || currentProblem.operands[i].type === 'mixed')) {
                    // Check if already has parentheses (from formatOperandText)
